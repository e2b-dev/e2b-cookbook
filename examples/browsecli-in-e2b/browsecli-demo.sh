#!/usr/bin/env bash
#
# browsecli-demo.sh — the load-bearing demo this E2B template runs.
#
# Inside the E2B sandbox, this script uses the `browse` CLI to drive a *remote*
# Browserbase session. The browser never runs in the sandbox — the sandbox runs
# the agent/CLI and connects out over CDP to a Verified Browserbase browser that:
