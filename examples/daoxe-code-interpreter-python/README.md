# DaoXE + E2B Code Interpreter (Python)
...
DaoXE is a **multi-model, multi-protocol** API gateway. This example uses the **OpenAI-compatible Chat Completions** surface via the official OpenAI Python SDK with a custom base URL:
...
Open `daoxe_code_interpreter.ipynb` and run all cells (or run the final `main()` cell after installing deps).
...
1. Creates an OpenAI client with `base_url=https://daoxe.com/v1` and your DaoXE key.
2. Asks a model (exact `DAOXE_MODEL` from your account) to solve a small data-viz task with function calling.
3. Executes the generated Python in an E2B sandbox via the `execute_python` tool.
4. Optionally saves a chart as `height_distribution.png`.
