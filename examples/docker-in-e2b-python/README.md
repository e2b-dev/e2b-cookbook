# How to Build and Run Docker in E2B Sandbox (Python)

This example shows you how to build your own E2B [sandbox template](https://e2b.dev/docs/sandbox-template) that supports running Docker containers inside an E2B sandbox using Python.
...
```bash
poetry run python -m template.build_dev
```
...
```bash
poetry run python -m template.build_prod
```

Then update `main.py` to use `"e2b-with-docker"` instead of `"e2b-with-docker-dev"`.
...
The custom template (defined in `template/template.py`) includes:
...
You can customize the template by editing `template/template.py`:
