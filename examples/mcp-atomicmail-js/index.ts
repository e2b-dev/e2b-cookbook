...
// E2B's MCP gateway namespaces tools with the server key, so the Atomic Mail
// `register` tool is exposed as e.g. `github/Atomic-Mail/atomic-mail-agentic-register`.
// Resolve the real names from the tool list instead of hardcoding them.
function makeToolResolver(toolNames: string[]) {
  return (base: string): string => {
    const match =
      toolNames.find((name) => name === base) ??
      toolNames.find((name) => name.endsWith(`-${base}`) || name.endsWith(`/${base}`));
    if (!match) {
      throw new Error(`MCP server does not expose a "${base}" tool. Available: ${toolNames.join(', ')}`);
    }
    return match;
  };
}
...
async function callTool(
  ...
  // Proof-of-work signup and JMAP round trips can exceed the MCP SDK's default
  // 60s request timeout, so raise it per call.
...
async function run() {
  ...
  try {
    ...
    // The service returns the inbox as a local part; make it a full address.
    const inboxAddress = inbox.includes('@') ? inbox : `${inbox}@atomicmail.ai`;
    console.log(`Inbox registered: ${inboxAddress}`);
    ...
    const recipient = process.env.RECIPIENT_EMAIL || inboxAddress;
    const isSelfSend = recipient.toLowerCase() === inboxAddress.toLowerCase();

    // A unique marker so we can spot exactly this message when polling.
    // Keep SUBJECT/BODY single-line: preset values are substituted verbatim into
    // the JSON template, so raw newlines would produce invalid JSON.
    ...
    await callTool(client, jmapTool, {
    ...
    if (!isSelfSend) {
      ...
      for (let attempt = 1; attempt <= 10 && !delivered; attempt++) {
        await sleep(6000);
        const { text } = await callTool(client, jmapTool, { ops_file: 'list_inbox.json' });
        ...
        if (match) {
          ...
          console.log(`  From:    ${match.from?.[0]?.email ?? inboxAddress}`);
