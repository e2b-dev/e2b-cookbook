...
function mistralMessages(newMessages: Message[]): Message[] {
  return [
    { role: 'system', content: 'You are a skilled AI developer that does not make typos and basic mistakes. You have access to a safe cloud computer running Ubuntu. You can run python code, use terminal (you are running in a non-interactive mode), use filesystem, and have access to the internet. Work inside the "/home/user" directory and always specify the full path when making filesystem operations. Always respond in markdown and always specify whether the code block is python or shell by using `py` and `sh`.' },
...
export async function POST(req: Request) {
  ...
  if (sandboxID && lastUsed && now - lastUsed < TTL) {
    ...
    sbx = await CodeInterpreter.create({
      logger: console,
      onStdout: out => console.log('[sandbox stdout]', out),
      onStderr: out => console.log('[sandbox stderr]', out),
    })
